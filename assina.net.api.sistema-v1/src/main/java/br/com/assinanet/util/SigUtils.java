...
public class SigUtils {
    ...
    public static <T> Optional<T> verifica(T obj) {
        if (obj == null) {
            ...
            return s.length() != 0 && !s.equalsIgnoreCase("null") ? Optional.of(obj) : Optional.empty();
        }
    }
    ...
    public static PDSignature getLastRelevantSignature(PDDocument document) throws IOException {
        ...
        Iterator var2 = document.getSignatureDictionaries().iterator();

        while (var2.hasNext()) {
            PDSignature signature = (PDSignature) var2.next();
